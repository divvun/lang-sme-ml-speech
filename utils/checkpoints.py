...
def get_checkpoint_paths(checkpoint_type: str, paths: Paths):
    ...
    elif checkpoint_type == 'asr_forward':
        weights_path = paths.asr_forward_latest_weights
        optim_path = paths.asr_forward_latest_optim
        checkpoint_path = paths.asr_forward_checkpoints
...
def save_checkpoint(checkpoint_type: str, paths: Paths, model, optimizer, *,
    ...
    def helper(path_dict, is_named):
        ...
        if str(path_dict['w']).endswith('asr_latest_weights.bin'):
            print('here')
