...
def get_tts_datasets(path: Path, batch_size, r, model_type='tacotron'):
    if model_type=='forward_dt':
        # print(path)
        train_data = unpickle_binary(path/'train_dataset_dt.pkl')
        text_dict = unpickle_binary(path/'text_dict_dt.pkl')
        train_len_original = len(train_data)
        # print(len(train_data))
        # print(len(text_dict))
        # print(train_len_original)

    if model_type=='forward_pre_dt':
        train_data = unpickle_binary(path/'train_dataset_dt.pkl')
        text_dict = unpickle_binary(path/'text_dict_dt.pkl')
        train_len_original = len(train_data)

    if model_type!='forward_pre_dt' and model_type!='forward_dt':
        train_data = unpickle_binary(path/'train_dataset.pkl')
    ...
    if model_type=='forward_dt' and hp.forward_filter_attention:
    ...
    elif model_type=='forward_pre_dt':
        train_dataset = TacoDataset(path, train_ids, text_dict)
    elif model_type == 'forward_dt':

        train_dataset = ForwardDataset(path, train_ids, text_dict)
    else:
        raise ValueError(f'Unknown model: {model_type}, must be either [tacotron, forward, forward_dt]!')
    ...
    if model_type == 'forward' or model_type=='tacotron':
        train_set = DataLoader(train_dataset,
                           collate_fn=lambda batch: collate_tts(batch, r),
                           batch_size=batch_size,
                           sampler=train_sampler,
...
def filter_bad_attentions(dataset: List[tuple], attention_score_dict: Dict[str, tuple]) -> List[tuple]:
    dataset_filtered = []
    for item_id, mel_len in dataset:
        if item_id in attention_score_dict:
