...
class BatchNormConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel, activation=None):
        ...
        # self.conv_t = nn.Conv1d(in_channels, out_channels, 1, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        # except:
        #     # conv = nn.Conv1d(in_channels, out_channels, 4, stride=1, padding=4 // 2, bias=False)
        #     x = self.conv_t(x)
        if self.activation:
            x = self.activation(x)
        x = self.bnorm(x)
