...
from datasets import Dataset as Dataset_asr
...
class ForwardTrainer:
    ...
    def train(self, model_tts: ForwardTacotron, model_asr: Wav2Vec2ForCTC, optimizer_tts: Optimizer, optimizer_asr: Optimizer) -> None:
        ...
        asr_train_set  = pd.read_parquet('./data/speech-sme-asr/train_asr.parquet')
        asr_train_set = Dataset_asr.from_pandas(asr_train_set)
        asr_test_set = pd.read_parquet('./data/speech-sme-asr/test_asr.parquet')
        asr_test_set = Dataset_asr.from_pandas(asr_test_set)
        # asr_test_set = 
        # exit()
    ...
    def train_session(self, model_tts: ForwardTacotron, model_asr: Wav2Vec2ForCTC, optimizer_tts: Optimizer, tts_session: ForwardSession, asr_session: ASRSession, asr_trainer, optimizer_asr) -> None:
        ...
        for e in range(1, epochs + 1):
            ...
            for step, inputs in enumerate(asr_session.train_set):
                ...
                outputs = model_asr(input_ids, attention_mask=attention_mask, labels=labels)
                asr_s_loss = outputs[0]
                asr_s_loss.backward()
            ...
            # else:
            #     tts_s_loss.to(device)
            #     model_tts.to(device)
            #     tts_s_loss.backward()
            #     for state in optimizer_tts.state.values():
            #         for k, v in state.items():
            #             # print(v)
            #             if torch.is_tensor(v):
            #                 state[k] = v.to(device)
    ...
    def dual_transform(self, model_tts, model_asr, optimizer_tts, optimizer_asr, asr_test_set, m_loss_avg, dur_loss_avg, device, asr_current_step, e, epochs, duration_avg, total_iters, tts_s_loss, asr_s_loss, tts_lr, tts_dt_path):
            ...
            for i, x in enumerate(inputs, 1):
                _, m, dur = model_tts.generate(x, alpha=.7)            
                m = torch.tensor(m).unsqueeze(0)

                wav_path = waveglow_generate(m, text[i-1], tts_k=e)
